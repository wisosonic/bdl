...
class DocumentResource extends Resource
{
    ...
    {
        $columns = Schema::getColumns((new Document())->getTable());
        $tableColumns = [];
        foreach ($columns as $column) {
            if (in_array($column["name"], ['id', 'url', 'created_at', 'updated_at'])) {
                continue;
            }
        ...
        $tableColumns[] = FileUpload::make('url')
                ->label('URL')
                ->directory('/uploads/lda') // Optional: folder in storage
